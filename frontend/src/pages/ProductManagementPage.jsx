...
export function ProductManagementPage() {
  ...
  }, [isModalOpen, newProductName, selectedCreateCategory, isSkuManuallyEdited, newProductSku]);
    ...
    <section className="space-y-5">
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="hidden grid-cols-12 border-b border-slate-200 bg-slate-50 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500 md:grid">
          <span className="col-span-4">Tên sản phẩm</span>
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-slate-100">
            {filteredProducts.map((product) => (
              <div key={product.id} className="grid gap-2 px-4 py-4 md:grid-cols-12 md:items-center">
                ...
                <p className="text-sm text-slate-700 md:col-span-3">{getCategoryName(product, categories)}</p>
          ...
          <p className="px-4 py-6 text-sm text-slate-500">Không tìm thấy sản phẩm.</p>
        )}
      </div>
