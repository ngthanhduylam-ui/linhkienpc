-- Review this migration and take the normal production backup before later execution.
...
INSERT INTO print_template_settings (
  document_type,
  active_template,
  custom_template_config,
  custom_template_schema_version
)
SELECT 'sale_delivery_note', 'system', NULL, NULL
WHERE NOT EXISTS (
  SELECT 1
  FROM print_template_settings
  WHERE document_type = 'sale_delivery_note'
);
